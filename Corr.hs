...
comb = [(x,y) | x <- [0..8], y <- [x + 1..8]]


tracoo (x,y) (xc,yc) = (1 / 9 * (xc + fromIntegral x), 1 / 4 * (yc + fromIntegral y))

points = zipWith tracoo comb

picture c _ = Pictures . map (pointToPicture $ radium c) . points 

pointToPicture r (x,y) = Translate x y $ Circle r
